...
export default function CsvToJsonTool() {
    ...
    const convertCsvToJson = (csvStr: string, currentDelimiter: ',' | ';' | '\t' = delimiter) => {
        setInput(csvStr);
        setError('');
        ...
        try {
            const results = Papa.parse(csvStr, {
                header: true,
                skipEmptyLines: true,
    ...
    const handleDelimiterChange = (newDelimiter: ',' | ';' | '\t') => {
        ...
        convertCsvToJson(input, newDelimiter);
    };
        ...
        <div className="p-6 md:p-10 flex flex-col gap-8">
            <div className="flex bg-zinc-100 dark:bg-zinc-900 p-1.5 rounded-2xl border border-zinc-200 dark:border-zinc-800 w-fit">
                <button
                    ...
                    className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${delimiter === ',' ? 'bg-white dark:bg-zinc-800 shadow-sm text-brand-primary' : 'text-zinc-500'}`}
                >
                    Comma ( , )
                </button>
                <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                        ...
                        {input && (
                            <button onClick={() => convertCsvToJson('')} className="text-zinc-400 hover:text-rose-500 transition-colors">
                                <Trash2 size={16} />
                            </button>
                        )}
                    </div>
...
2,Jane,jane@example.com"
                        value={input}
                        onChange={(e) => convertCsvToJson(e.target.value)}
                    />
                </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-6 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-100 dark:border-zinc-800 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">
                <div className="flex gap-4">
                    <span>Format: JSON (RFC 8259)</span>
                     <span className="hidden sm:inline">•</span>
                    <span>Delim: {delimiter === '\t' ? 'Tab' : `'${delimiter}'`}</span>
                </div>
